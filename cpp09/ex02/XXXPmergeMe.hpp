...
class PmergeMe {
	...
	private:
		...
		std::vector<std::pair<int, int> >	reorderPairsBySortedWinners(std::vector<std::pair<int, int> > pairVector, std::vector<int> sortedWinners);
	...
	// std::vector<int>	sortByFordJohnsonVector(std::vector<int> vecInProgress);

	// std::vector<Node*>	buildNodes(std::vector<int> vecInProgress);
	// std::vector<Node*>	sortWinnerNodes(std::vector<Node*> nodes);
	// std::vector<int>	buildSortedChain(std::vector<Node*> sortedWinners, Node *oddNode);

	// void				collectLosers(Node *node, std::vector<Node*> &losers);
	// size_t				findPartnerBound(std::vector<int> &mainChain, int partnerValue);
